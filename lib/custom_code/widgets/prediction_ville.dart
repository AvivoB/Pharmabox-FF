...
class PredictionVille extends StatefulWidget {
  PredictionVille({Key? key, required this.onPlaceSelected, this.initialValue}) : super(key: key);
...
class _PredictionVilleState extends State<PredictionVille> {
  ...
  void _onSearchChanged(String query) async {
    if (query.isNotEmpty) {
      final response = await http.get(Uri.parse('https://maps.googleapis.com/maps/api/geocode/json?address=$query&region=fr&key=$googleMapsApi'));
      ...
      if (json['status'] == 'OK') {
        setState(() {
          ...
          _predictions = json['results']
              .map((result) {
                final localityComponent = result['address_components'].firstWhere((component) => component['types'].contains('locality') as bool, orElse: () => null);

                final postalCodeComponent = result['address_components'].firstWhere((component) => component['types'].contains('postal_code') as bool, orElse: () => null);

                if (localityComponent != null && postalCodeComponent != null) {
                  return {'city': localityComponent['long_name'], 'postal_code': postalCodeComponent['long_name']};
                }
                return null;
              })
          ...
          Container(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  title: Text(_predictions[index]['postal_code'] + ', ' + _predictions[index]['city'], style: FlutterFlowTheme.of(context).bodyMedium),
                  onTap: () {
